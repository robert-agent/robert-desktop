...
pub async fn init_backend() -> Result<RobertState, String> {
    ...
    let data_dir = dirs::home_dir().unwrap_or(PathBuf::from(".")).join(".robert/data");
    std::fs::create_dir_all(&data_dir).map_err(|e| e.to_string())?;
    
    let store = SurrealStore::new(data_dir).await.map_err(|e| e.to_string())?;

    // 3. Initialize Ingestion Pipeline
    let ingestion_pipeline = Arc::new(IngestionPipeline::new(store.clone()).map_err(|e| e.to_string())?);
